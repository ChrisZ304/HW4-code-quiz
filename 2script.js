...
// if user clicks startQuiz button
start_btn.onclick = ()=>{
    info_box.classList.add("activeInfo"); //show info box
}

// if user clicks exitQuiz button 
exit_btn.onclick = ()=>{
    info_box.classList.remove("activeInfo"); //hide info box
}

// if user clicks continueQuiz button 
...
// if user clicks restartQuiz button
...
// if user clicks quitQuiz button
quit_quiz.onclick = ()=>{
...
// if user clicks Next Que button
next_btn.onclick = ()=>{
...
function optionSelected(answer){
    ...
    next_btn.classList.add("show"); // if user selected an option show next button
}
...
function showResult(){
    ...
    if (userScore > 3){ // if user scored more than 3
        ...
        let scoreTag = '<span>Congratulaions! You got: <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        ...
        let scoreTag = '<span>Sorry, You only got: <p>'+ userScore +'</p> out of <p>' + questions.length +'.</p></span> Better luck next time!';
        scoreText.innerHTML = scoreTag;
    }
}
...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 0){ //if timer is less than 0
            clearInterval(counter); //clear counter
            timeText.textContent = "Time is up!"; //change the time text to time is up
            const allOptions = option_list.children.length; //getting all option items
            let correctAns = questions[que_count].answer; //getting correct answer from array
            for(i=0; i < allOptions; i++){
                if(option_list.children[i].textContent == correctAns){ //if there is an option which is matched to an array answer
...
function queCounter(index){
    ...
    let totalQuesCountTag = '<span><p>'+ index +'</p> / <p> '+ questions.length +'  </p>Questions</span>';
